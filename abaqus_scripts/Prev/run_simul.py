...
#%% UTILITIES
# =============================================================================
# The pure-Python helpers below are SHARED with the GUI (which runs Python
# 3.x) and therefore live in abaqus_scripts/cel_common.py. Keeping ONE
# implementation removes the hand-synced duplication of
# resolve_tool_translation, whose two copies had already drifted apart.
#
# Import strategy: try a plain import first (Abaqus may already have this
# script's directory on sys.path). Only if that fails do we augment sys.path,
# deriving the directory from __file__ when it is defined and falling back to
# the noGUI= argument, since __file__ is not guaranteed under `abaqus cae
# noGUI=`.
try:
    from cel_common import (cfg_get, discretize, solve_tool_dimensions,
                            resolve_mapping, resolve_tool_translation,
                            ToolGeometryError)
except ImportError:
    _here = None
    try:
        ...
        for _a in sys.argv:
            if _a.startswith('noGUI=') or _a.endswith('run_simul.py'):
                _cand = _a.split('=', 1)[-1]
                if os.path.isfile(_cand):
                    _here = os.path.dirname(os.path.abspath(_cand))
                    break
    if _here is None:
        raise ImportError(
            'cannot locate cel_common.py: run_simul.py must sit next to it '
            '(abaqus_scripts/), and its directory could not be derived from '
            '__file__ nor from sys.argv')
