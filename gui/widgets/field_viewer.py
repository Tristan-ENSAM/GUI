...
class FieldViewer(QWidget):
    ...
    def set_mesh(self, nodes_xy: np.ndarray, element_face_indices: np.ndarray):
        ...
        """
        # Clear any previous patch collection and colorbar
        ...
        # Fit the axes to the mesh extent (with a small margin)
        ...
        # Initial colorbar (empty range — refreshed on set_values)
    ...
    def set_values(self, values: np.ndarray, vmin: float = None,
        ...
        self._polys.set_array(np.asarray(values, dtype=np.float32))
        if cmap is not None:
